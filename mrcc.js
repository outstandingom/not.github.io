...
function validateUTR(utr) {
    // UTR numbers are typically 12-16 digits
    const utrRegex = /^[0-9]{8,16}$/;
...
function getCurrentLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
        (position) => {
            ...
            // Update display
            latitudeDisplay.textContent = currentLatitude.toFixed(6);
            longitudeDisplay.textContent = currentLongitude.toFixed(6);
            ...
            getLocationBtn.innerHTML = '<i class="fas fa-check me-1"></i> Location Captured';
        },
        (error) => {
            ...
            setLoading(false);
            getLocationBtn.disabled = false;
            getLocationBtn.innerHTML = '<i class="fas fa-location-arrow me-1"></i> Get Current Location';
        },
        {
...
// Update map preview with coordinates
function updateMapWithCoordinates() {
    if (currentLatitude && currentLongitude && mapPreview) {
        mapPreview.innerHTML = `
            <iframe 
                src="https://maps.google.com/maps?q=${currentLatitude},${currentLongitude}&z=15&output=embed" 
                width="100%" 
                height="100%" 
...
function validateForm() {
    ...
    // Validate UTR number
    const utr = document.getElementById('registrationUtr').value.trim();
    if (!utr || !validateUTR(utr)) {
        document.getElementById('registrationUtr').classList.add('is-invalid');
        isValid = false;
    }
    ...
    if (!currentLatitude || !currentLongitude) {
        showToast('Please get your location coordinates', true);
        isValid = false;
    }
...
form.addEventListener('submit', async (e) => {
    ...
    const salonData = {
        ...
        status: 'pending',
        // New fields for location and payment
        latitude: currentLatitude,
        longitude: currentLongitude,
        registration_fee_paid: true,
        registration_utr: document.getElementById('registrationUtr').value.trim(),
...
form.addEventListener('input', (e) => {
    ...
    // Special validation for UPI ID
    if (target.id === 'upi_id') {
        if (validateUPI(target.value.trim())) {
            target.classList.remove('is-invalid');
        } else {
            target.classList.add('is-invalid');
        }
    ...
    // Special validation for UTR
    if (target.id === 'registrationUtr') {
        if (validateUTR(target.value.trim())) {
            target.classList.remove('is-invalid');
        } else {
            target.classList.add('is-invalid');
        }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    setTimeout(() => {
        showToast('Please allow location access when prompted for better service visibility');
    }, 2000);
});